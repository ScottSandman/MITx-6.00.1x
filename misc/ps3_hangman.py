...
def getGuessedWord(secretWord, lettersGuessed):
    ...
    for char in secretWord:
        if char in lettersGuessed:
            blank[index] = secretWord[index]
            blank[index] = char
        index += 1
    blankString = " ".join(blank)    
    return blankString
...
def getAvailableLetters(lettersGuessed):
    ...
    # FILL IN YOUR CODE HERE...
    import string
    aLetters = list(string.ascii_lowercase)
    aLettersCopy = aLetters[:]
    for char in aLettersCopy:
        if char in lettersGuessed:
            aLetters.remove(char)
    aString = "".join(aLetters)
    return aString            
